...
class mce(object):
    ...
    **IMPORTANT**
    ...
    def readIntPoint(self, command, isPoint=True):
        ...
        point = [int(floor(x)) for x in point]
        return Vector(*point)
    ...
    def readBlockInfo(self, command):
        ...
        if len(matches):
            ...
            try:
                data = 0
                if ':' in keyword:
                    blockID, data = [int(x) for x in keyword.split(':')]
                else:
                    blockID = int(keyword)
    ...
    # TODO: can I rewrite this so that it returns an iterator instead of a list for efficient memory usage?
    ...
    '''
        ...
        destPoint = [int(floor(x)) for x in destPoint]
    ...
    '''
        ...
        if len(command):
            if len(command) > 1:
                rx, rz = [int(x) for x in command[:2]]
    ...
    '''
        if len(command):
            box = self.readBox(command)
            # TODO: Can I rewrite this more efficiently?
